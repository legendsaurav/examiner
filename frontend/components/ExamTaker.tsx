...
import { User, ChevronLeft, ChevronRight, Info, Delete } from 'lucide-react';
...
const VirtualKeypad: React.FC<{ 
    ...
    const handleKeyPress = (key: string) => {
        if (key === 'BACKSPACE') {
            ...
            // Prevent multiple decimals or minus signs in weird places if wanted, 
            // but simple appending is usually standard for these raw inputs.
            // Let's do simple appending.
            if (value.length < 10) { // Max length limit
                onChange(value + key);
            }
        }
    };
    ...
    return (
        <div className="mt-6 max-w-xs mx-auto md:mx-0">
             <div className="mb-2">
                 <label className="block text-sm font-semibold text-slate-700 mb-1">Your input</label>
                 <div className="flex gap-2">
                     <div className="flex-1 h-12 bg-white border-2 border-slate-300 rounded-md flex items-center px-4 text-xl font-mono text-slate-800 shadow-inner">
                         {value}<span className="animate-pulse text-brand-500">|</span>
                     </div>
                     <button 
                        onClick={() => handleKeyPress('BACKSPACE')}
                        className="w-16 bg-blue-600 text-white rounded-md flex items-center justify-center hover:bg-blue-700 active:bg-blue-800 shadow-sm"
                     >
             ...
             <div className="grid grid-cols-3 gap-2 mb-2">
                 {keys.map(key => (
                     <button
                        ...
                        className="h-12 bg-blue-600 text-white text-xl font-bold rounded-md hover:bg-blue-700 active:bg-blue-800 shadow-sm transition-colors"
                     >
                         {key}
                     </button>
                 ))}
             ...
             <button 
                ...
                className="w-full h-12 bg-blue-700 text-white text-lg font-bold rounded-md hover:bg-blue-800 active:bg-blue-900 shadow-md tracking-wider uppercase"
             >
                 Enter
             </button>
        </div>
...
export const ExamTaker: React.FC<ExamTakerProps> = ({ exam, onComplete, onExit }) => {
  ...
  const [answers, setAnswers] = useState<Record<string, { type: 'MCQ' | 'INTEGER', value: string }>>({}); 
  ...
  const handleMCQSelect = (optionId: string) => {
    const qId = exam.questions[currentQuestionIndex].id;
    setAnswers(prev => ({ ...prev, [qId]: { type: 'MCQ', value: optionId } }));
  };

  const handleIntegerInput = (val: string) => {
    const qId = exam.questions[currentQuestionIndex].id;
    setAnswers(prev => ({ ...prev, [qId]: { type: 'INTEGER', value: val } }));
  };
  ...
  const handleSaveAndMarkForReview = () => {
    ...
    if (answers[qId]) {
      ...
      alert("Please select an option or enter a value to Save & Mark for Review");
      return;
    }
    if (currentQuestionIndex < exam.questions.length - 1) {
      setCurrentQuestionIndex(prev => prev + 1);
        ...
        <div className="max-w-5xl mx-auto p-6">
          ...
          <div className="space-y-4 text-slate-800 text-sm leading-relaxed border p-6 rounded shadow-sm bg-white">
            ...
            <ol className="list-decimal list-inside space-y-2">
              ...
              <li>Questions can be MCQ (Multiple Choice) or Integer Type (Numerical Input).</li>
              <li>Use the virtual keypad for Integer type questions.</li>
            </ol>
  ...
  const getStatusColor = (status: QuestionStatus) => {
    switch (status) {
      ...
      case 'not_answered': return 'bg-orange-600 text-white';
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <div className="bg-white border-b border-slate-200 p-3 flex justify-between items-center shrink-0">
                    <h2 className="text-lg font-bold text-slate-800">Question {currentQuestionIndex + 1}: <span className="text-sm font-normal text-slate-500 ml-2">({currentQuestion.type === 'INTEGER' ? 'Integer Type' : 'Multiple Choice'})</span></h2>
                        ...
                        <div className="text-lg text-slate-900 leading-relaxed font-medium mb-8 border-b pb-8 whitespace-pre-wrap">
                            {currentQuestion.text}
                             ...
                             {currentQuestion.imageUrl && (
                                <div className="mt-4">
                                    <img src={currentQuestion.imageUrl} alt="Question Diagram" className="max-w-full max-h-96 object-contain rounded border border-slate-200" />
                                </div>
                            )}
                        </div>
                                    ...
                                    >
                                        <div className="relative flex items-center justify-center shrink-0 mt-0.5">
                                            <input 
                                                ...
                                                checked={answers[currentQuestion.id]?.value === option.id}
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            {option.imageUrl && (
                                                <img src={option.imageUrl} alt={`Option ${idx + 1}`} className="mt-2 max-w-full max-h-48 object-contain rounded border border-slate-200" />
                                            )}
                                        </div>
                                    </label>
                                ))}
            ...
            <div className="w-80 bg-blue-50 border-l border-slate-300 flex flex-col h-full shrink-0 overflow-y-auto custom-scrollbar">
                <div className="p-4 border-b border-blue-100 bg-white">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px] leading-tight text-slate-600">
                        ...
                        <div className="col-span-2 flex items-center gap-1.5"><div className="w-5 h-5 bg-purple-700 text-white rounded-full flex items-center justify-center font-bold relative"><div className="absolute bottom-0 right-0 w-1.5 h-1.5 bg-green-400 rounded-full border border-white"></div>{counts.answered_marked_for_review}</div> Ans & Marked for Review</div>
                    </div>
                </div>
